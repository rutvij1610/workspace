...
class ShiftRegister {
    ...
    public:
		...
		void binarytohexadecimal(string bits){
			if(reg.empty())
				return;
			long int decimal_number = binarytodecimal(bits);
...
int main(int argc, char** argv)
{
    ShiftRegister sr;
	ShiftRegister sr2;
	string binary1 = "000000";
	string binary2 = "000000";
	long int a;
	long int b;
	long int c;
    for(int i = 1;i < argc; i++) {
        ...
        if(cmd == "-i") {
			...
			i++;
			long int a = sr.binarytodecimal(binary1);
			long int b = sr2.binarytodecimal(binary2);
				if(argv[i] == "+"){
					 c = a + b;
